...
class AdamWConfig(BaseModel):
    ...
    eps: float = Field(1e-8, description="Term added to the denominator to improve numerical stability")
    weight_decay: float = Field(1e-2, description="Weight decay coefficient. In AdamW, this is decoupled from the gradient update")
...
class AdamConfig(BaseModel):
    ...
    eps: float = Field(1e-8, description="Numerical stability term.")
...
class LBFGSConfig(BaseModel):
    ...
    tolerance_grad: float = Field(1e-7, description="Termination tolerance on first order optimality")
    tolerance_change: float = Field(1e-9, description="Termination tolerance on function value/parameter changes")
